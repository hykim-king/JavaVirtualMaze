...
public class ReservationDao implements WorkDiv<ReservationVO>, PLogger {
	...
	public int doSave(ReservationVO param) {
		...
		flag = reservation.add(param)? 1 : 0; 
	 ...
	 */
	public int doUpdate(ReservationVO param, ReservationVO updateParam) {
		int flag = 0;
		
		
		for(ReservationVO vo : reservation)
		{
			// isSame 메서드를 통해 리스트의 수정할 예약정보 비교해서 찾고 수정한 예약정보를 가져와 set으로 변경
			if(null != vo && vo.isSame(param))
			{
				vo.setDate(updateParam.getDate());
				vo.setTheme(updateParam.getTheme());
				flag = 3;
				return flag;
			}			
		}
