...
public class ReservationService {
	...
	public int deleteReservation(ReservationVO param) {
		...
		if (false == isExistsMobileNumber(param)) {
			flag = 2;
			return flag; // false -> 중복번호 없음
		}

		flag = dao.doDelete(param); // 중복된 번호 있으니까 삭제 실행
	...
	public int updateReservation(ReservationVO param) {
		...
		flag = dao.doUpdate(param); // 중복 번호 있으니까 수정
	...
	public boolean isExistsDateandThemename(ReservationVO param) {
		...
		for (ReservationVO vo : list) {
			...
			// 예약된 테마인지 확인
			if (vo.getDate().equals(param.getDate()) && vo.getTheme().equals(param.getTheme())) {
				return true; // true -> 예약된테마
			}
		}
